...
export function normalizeSkill(s) {
  ...
  const pass = s.last_run_passes == null ? 0.5 : (s.last_run_passes ? 0.92 : 0.35);
  return {
    id: s.name,
    family: 'skill',
...
export function normalizeTask(t) {
  return {
    ...
    label: t.label || '—',
    skill: Array.isArray(t.skills_used) && t.skills_used.length > 0 ? t.skills_used[0] : '—',
    mode: t.mode || 'unknown',
    helix_id: null,
    helix_phase: null,
...
export function normalizeHelixRun(r) {
  ...
  let current = null;
  if (r.status === 'running' && r.phases_planned && r.phases_planned.length > 0) {
...
export async function fetchLive() {
  try { return await apiFetch('/api/live'); } catch { return null; }
}

export async function fetchSessions() {
  try { return await apiFetch('/api/sessions'); } catch { return []; }
}

export async function fetchSessionDetail(id) {
  try { return await apiFetch('/api/sessions/' + id); } catch { return null; }
}

export async function fetchEvolution() {
  try { return await apiFetch('/api/evolution'); } catch {
    return { progress: [], findings: [], git_log: [], helix_runs: [], helix_latest: null };
  }
}
...
export async function fetchIntel() {
  try { return await apiFetch('/api/intel'); } catch {
    return { findings: [], progress: [] };
  }
}

export async function fetchHealth() {
  try { return await apiFetch('/api/health'); } catch { return null; }
}
