...
const { Tag, StatusDot, Sparkline, fmtMs, fmtNum, fmtPct, relTime } =
  window.__C;
...
function Sidebar() {
  ...
  const items = [
    { id: "live", glyph: "◉", label: "Live", badge: "●" },
    {
      ...
      glyph: "▣",
      label: "Sessions",
      badge: health.sessions || null,
    },
    {
      id: "insights",
      glyph: "✦",
      label: "Insights",
      badge: kpis.unread_actions,
    },
  ...
  return html`
    <aside class="sidebar">
      ...
      <div class="nav-item" onClick=${() => $cmdkOpen.set(true)}>
        <span class="glyph">⌘</span><span>搜索</span
        ><span class="badge">⌘K</span>
      </div>
      <div class="nav-item">
        <span class="glyph">⤓</span><span>导出今日</span>
      </div>
      <div class="nav-item"><span class="glyph">⚙</span><span>设置</span></div>

      <div
        style="margin-top:auto; padding:14px 12px; font-family:var(--ff-mono); font-size:10px; color:var(--t4); letter-spacing:.04em"
      >
        v0.7.0 · ${platform}<br />
        Node stdlib · jsonl files<br />
    ...
    <header class="topbar">
      ...
      <div class="search" onClick=${() => $cmdkOpen.set(true)}>
        ...
        <span class="placeholder"
          >搜索 finding · progress · event · skill · commit ...</span
        >
      ...
      <div class="right">
        <div class="pill">
          <span
            ...
            style=${sseOk
              ? ""
              : "background:var(--st-warn); box-shadow:0 0 8px var(--st-warn)"}
          ></span>
          <span>${sseOk ? "SSE 已连" : "SSE 断开"}</span>
        </div>
        <span
          >rate
          <span class="mono" style="color:var(--t1)"
            >${health.event_rate.toFixed(2)}/s</span
          ></span
        >
        <span
          >mem
          <span class="mono" style="color:var(--t1)"
            >${health.mem_mb.toFixed(0)}MB</span
          ></span
        >
      </div>
    </header>
  ...
  return html`
    <footer class="statusbar">
      <div class="item">
        ...
        ><span class="val">${health.sessions || 0}</span>
      </div>
      <div class="item">
        <span class="lbl">tasks:</span
        ><span class="val">${fmtNum(health.tasks || 0)}</span>
      </div>
      <div class="item">
        <span class="lbl">jsonl:</span
        ><span class="val">${health.db_size_mb.toFixed(2)}MB</span>
      </div>
      <div class="item">
        <span class="lbl">mem:</span
        ><span class="val ok">${health.mem_mb.toFixed(0)}MB</span>
      </div>
      <div class="item">
        <span class="lbl">rate:</span
        ><span class="val ok">${health.event_rate.toFixed(2)}/s</span>
      </div>
      <div class="item">
        <span class="lbl">skills:</span
        ><span class="val"
          >${health.skills_count || kpis.active_skill_count || 0}</span
        >
      </div>
      <div class="item">
        <span class="lbl">sse:</span
        ><span class=${"val " + (sseOk ? "ok" : "warn")}
          >${sseOk ? "●" : "○"}</span
        >
      </div>
      <div class="spacer"></div>
      <div class="item">
        <span class="lbl">last_event:</span
        ><span class="val mono"
          >${events[0]
            ? new Date(events[0].ts_ms).toLocaleTimeString("zh-CN", {
                hour12: false,
              })
            : "—"}</span
        >
...
function CmdK() {
  ...
  return html`
    <div
      class="cmdk-overlay"
      onClick=${(e) => e.target === e.currentTarget && $cmdkOpen.set(false)}
    >
      <div class="cmdk">
        <div class="cmdk-input-row">
          ...
          <input
            ref=${inputRef}
            class="cmdk-input"
            placeholder="搜索 · 用 kind:finding 等过滤"
            value=${q}
            onInput=${(e) => {
              setQ(e.target.value);
              setSel(0);
        ...
        <div class="cmdk-results">
          ${results.length === 0
            ? html`<div class="cmdk-empty">
                无匹配 · 试试 a2-repo · helix · F-012 · skill: · commit:
              </div>`
            : results.map(
                (r, i) => html`
                  <div
                    class=${"cmdk-row" + (i === sel ? " active" : "")}
        ...
        <div class="cmdk-footer">
          ...
          <span style="margin-left:auto"
            >FTS5 · unicode61 · ${idx.length} 文档</span
          >
        </div>
      </div>
    </div>
...
function SkillDrawer() {
  ...
  const related = skillId
    ? findings.filter(
        (f) =>
          (f.body && f.body.includes(skillId)) ||
          (f.title && f.title.includes(skillId.split("-")[0])),
      )
    : [];

  const passColor = !skill
    ? ""
    : skill.pass < 0.5
      ? "var(--st-error)"
      : skill.pass < 0.8
  ...
  return html`
    <div
      class=${"drawer-overlay" + (skill ? " open" : "")}
      onClick=${() => $drawerSkill.set(null)}
    ></div>
    <div class=${"drawer" + (skill ? " open" : "")}>
      ...
      html`
        <div class="drawer-header">
          ...
          <button class="btn ghost" onClick=${() => $drawerSkill.set(null)}>
            ✕
          </button>
        </div>
        <div class="drawer-body">
          <div class="drawer-section">
            ...
            >
              <div
                class="kpi sub"
                style="background:var(--bg-sub); padding:14px"
              >
              ...
              >
                ...
                <div class="val" style=${"font-size:24px; color:" + passColor}>
                  ${(skill.pass * 100).toFixed(0)}%
                </div>
              </div>
              <div
                class="kpi sub"
              ...
              >
                ...
                <div class="val" style=${"font-size:18px; color:" + stateColor}>
                  ${skill.state}
                </div>
              </div>
            </div>
            ${skill.last_run_summary
              ? html`
                  <div
                    class="text-xs muted"
                    style="margin-top:10px; padding:8px; background:var(--bg-sub); border-radius:4px; line-height:1.5"
                  >
                    ${skill.last_run_summary}
                  </div>
                `
          ...
          <div class="drawer-section">
            ...
            ${recent.length === 0
              ? html`<div class="muted text-sm">
                  当前任务中尚未出现此 skill 的事件
                </div>`
              : html`
                  <table class="data">
                    <tbody>
                      ${recent.map(
                        (e) => html`
                          <tr>
                            <td class="mono text-xs muted">
                              ${new Date(e.ts_ms).toLocaleTimeString("zh-CN", {
                                hour12: false,
                              })}
                            </td>
                            ...
                            <td>
                              ${e.payload.status === "failed"
                                ...
                                : html`<${Tag} kind="live">ok<//>`}
                            </td>
                            <td class="text-xs muted">
                              ${e.payload.tool ? "tool=" + e.payload.tool : ""}
                            </td>
                          </tr>
                        `,
                      )}
          ...
          <div class="drawer-section">
            <h4>关联 findings</h4>
            ${related.length === 0
              ? html`<div class="muted text-sm">无关联 finding</div>`
              : related.map(
                  (f) => html`
                    <div
                      style="padding:10px; background:var(--bg-sub); border-radius:6px; margin-bottom:6px"
                    >
